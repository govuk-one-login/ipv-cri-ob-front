...
test.describe('Consent (English)', { tag: '@desktop' }, () => {
  ...
  test('bank friendly name is present in the copy and preserved after a validation error', async ({
    page
  }) => {
  ...
  test.describe('Keyboard navigation', () => {
    test('user can check consent and tab onwards to the alternate route link', async ({ page }) => {
    ...
    test('activating the error summary link focuses the consent checkbox', async ({ page }) => {
...
test.describe('Consent (English) mobile', { tag: '@mobile' }, () => {
  test('Continue button on the consent page displays alt copy on mobile', async ({ page }) => {
...
test.describe('Consent (Welsh) mobile', { tag: '@mobile' }, () => {
  test('Continue button on the consent page displays alt copy on mobile', async ({ page }) => {
    ...
    await page.getByRole('link', { name: 'Cymraeg' }).click()
    await expect(page.locator('html')).toHaveAttribute('lang', 'cy')
    await expect(page.getByRole('button', { name: /Maecenas dignissim tempus est/ })).toBeVisible()
  })
})
